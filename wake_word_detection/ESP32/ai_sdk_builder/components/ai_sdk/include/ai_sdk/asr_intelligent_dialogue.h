...
public:
     ...
     * - completed=true：当前 URL 正常播放完成
     * - completed=false：当前 URL 被中断或播放失败
     * - all_done=true：当前批次所有已排队 URL 都已结束
     */
     ...
     * @brief 设置 TTS 播放回调
     *
     * 该回调与 setCallbacks() 独立，专用于 TTS 播放状态通知。
     * 需在 initAudio() 后配合 setAutoPlayTts(true) 才会触发自动播放相关回调。
     *
     * @param cb 播放回调函数，可为 nullptr
     */
