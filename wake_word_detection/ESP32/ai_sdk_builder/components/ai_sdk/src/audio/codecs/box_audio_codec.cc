/**
 * @file box_audio_codec.cc
 * @brief ESP-BOX 音频编解码器驱动实现
 *
...
BoxAudioCodec::BoxAudioCodec(void* i2c_master_handle,
    ...
    // 初始化 ES8311（输出/DAC）
    // AudioConfig 使用 7-bit I2C 地址（行业标准），
    // esp_codec_dev 驱动要求 8-bit 格式（内部做 addr >> 1），此处左移转换。
    audio_codec_i2c_cfg_t i2c_cfg = {
        .port = (i2c_port_t)1,
        .addr = static_cast<uint8_t>(es8311_addr << 1),
    ...
    es8311_cfg.codec_mode = ESP_CODEC_DEV_WORK_MODE_DAC;  // 仅 DAC 模式
    ...
    // 初始化 ES7210（输入/ADC），同样需要 7-bit → 8-bit 转换
    i2c_cfg.addr = static_cast<uint8_t>(es7210_addr << 1);
...
void BoxAudioCodec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk,
    ...
    i2s_chan_config_t chan_cfg = {
        ...
        .dma_desc_num = AI_SDK_AUDIO_CODEC_DMA_DESC_NUM,
        .dma_frame_num = AI_SDK_AUDIO_CODEC_DMA_FRAME_NUM,
    ...
    // RX TDM 模式配置（用于 ES7210 ADC，4 通道）
