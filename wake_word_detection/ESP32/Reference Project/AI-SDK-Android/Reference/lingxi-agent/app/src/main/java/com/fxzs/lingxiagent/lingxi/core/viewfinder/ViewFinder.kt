...
import com.fxzs.lingxiagent.lingxi.core.utils.ViewNodeNotFoundException
import com.fxzs.lingxiagent.lingxi.core.utils.ensureActive
import com.fxzs.lingxiagent.lingxi.core.utils.ensureNotInterrupt
import com.fxzs.lingxiagent.lingxi.core.utils.whileWaitTime
import com.fxzs.lingxiagent.lingxi.core.viewfinder.FinderBuilderWithOperation.Companion.WAIT_MILLIS
...
abstract class ViewFinder<T : ViewFinder<T>>(
    ...
    suspend fun findFirst(
        ...
        maxDepth: Int? = 10,
        needActiveNode: Boolean = false
    ): ViewNode? {
        val rootNode = if(needActiveNode) ViewNode.activeWinNode() else startNode
        return traverseAllNode(rootNode, includeInvisible = includeInvisible, maxDepth = maxDepth)
    }
