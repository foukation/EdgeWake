...
public class AiWorkFragment extends BaseFragment<VMAiWork> {
    ...
    private TextView btn_meeting;
    private TextView btn_research;
    private TextView btn_ppt;
    private TextView btn_translate;
    private TextView btn_draw;
    private View item_ai_meeting;
    private View item_research;
    private View item_ai_ppt;
    private View item_simul_translate;
    private View item_ai_draw;
    private View item_ai_excel;
    private View item_ai_file;
...
//        ll_empty = view.findViewById(R.id.ll_empty);
        item_ai_meeting = view.findViewById(R.id.item_ai_meeting);
        item_research = view.findViewById(R.id.item_research);
        item_ai_ppt = view.findViewById(R.id.item_ai_ppt);
        item_simul_translate = view.findViewById(R.id.item_simul_translate);
        item_ai_draw = view.findViewById(R.id.item_ai_draw);
        item_ai_excel = view.findViewById(R.id.item_ai_excel);
        item_ai_file = view.findViewById(R.id.item_ai_file);
        iv_avatar = view.findViewById(R.id.iv_avatar);
//        btn_meeting = view.findViewById(R.id.btn_meeting);
//        btn_research = view.findViewById(R.id.btn_research);
//        btn_ppt = view.findViewById(R.id.btn_ppt);
//        btn_translate = view.findViewById(R.id.btn_translate);

//        applyQuickButtonStyle(item_ai_meeting);
//        applyQuickButtonStyle(item_research);
//        applyQuickButtonStyle(item_ai_ppt);
//        applyQuickButtonStyle(item_simul_translate);
//        applyQuickButtonStyle(item_ai_draw);
        item_ai_meeting.setOnClickListener(view1 ->{
            startActivity(new Intent(getActivity(), MeetingContainActivity.class));
            UXOptimizer.scaleIn(item_ai_meeting, 25, () -> {},0.9f);
        });
        item_research.setOnClickListener(view2 ->{
            //TODO: 深度研究
            getCatDetailListBean bean = new getCatDetailListBean();
            bean.setId(94);
            bean.setModelId(156);
            bean.setModelName("深度研究");
            bean.setBotId("depthResearch");
            bean.setPreInput("你好呀！我是您的智能研究助手，专注高效分析与精准洞察，无需复杂操作，一句话即可获得全面、可靠的研究支持。现在，请告诉我您想了解什么？我将为您呈现深度分析！");
            bean.setDescription("灵犀深度研究智能体，一句话生成深度报告，专业分析即刻拥有！无论是行业趋势、市场分析还是购物选品、游戏攻略，只需输入简单需求，深度研究智能体即可自动完成全网搜索、数据整合与报告撰写，为您提供全面、精准的研究成果。高效智能，助您决策无忧！");
            bean.setTips("深度研究");
            bean.setName("深度研究");
            bean.setMenuCatId(4);
            bean.setIcon("https://ppt-1320245968.cos.ap-guangzhou.myqcloud.com/140f7e1a89b30fe25b4ae24b52429c125df51c3e1f945df9e3a4e17682407be1.png");
            Intent intent = new Intent(getActivity(), AgentContainActivity.class);
            intent.putExtra(Constant.INTENT_TYPE, AgentContainActivity.TYPE_AGENT);
            intent.putExtra(Constant.INTENT_DATA2, bean);
            getActivity().startActivity(intent);
            UXOptimizer.scaleIn(item_research, 25, () -> {

            },0.9f);
        });
        item_ai_ppt.setOnClickListener(view3 -> {

            UXOptimizer.scaleIn(item_ai_ppt, 25, () -> {
                startActivity(new Intent(getActivity(), PptTopicInputActivity.class));
            },0.9f);
        });
        item_simul_translate.setOnClickListener(view4 -> {
            startActivity(new Intent(getActivity(), com.fxzs.lingxiagent.lingxi.translate.SimultaneousTranslateActivity.class));
            UXOptimizer.scaleIn(item_simul_translate, 25, () -> {}, 0.9f);});

        item_ai_draw.setOnClickListener(view5 -> {
            startActivity(new Intent(getActivity(), DrawingSelectActivity.class));
            UXOptimizer.scaleIn(item_ai_draw, 25, () ->{},0.9f);
        });
        item_ai_excel.setOnClickListener(view6 -> {
            startActivity(new Intent(getActivity(), AiExcelContainActivity.class));
//            String token = SharedPreferencesUtil.getAccessToken();
//            Timber.tag("TAG").d("token = "+token);
//            WebViewActivity.start(getActivity(), Constants.AI_EXCEL_HOME+token, "");
            UXOptimizer.scaleIn(item_ai_excel, 25, () ->{},0.9f);
        });

        item_ai_file.setOnClickListener(view7 -> {
            startActivity(new Intent(getActivity(), com.fxzs.lingxiagent.view.aifile.AiFileToolActivity.class));
            UXOptimizer.scaleIn(item_ai_file, 25, () ->{},0.9f);
        });
...
//        jumpToPptTopicInput(session.getTitle());
    ...
    public void onResume() {
        super.onResume();
        UXOptimizer.scaleIn(item_ai_meeting, 0,null,1.0f);
        UXOptimizer.scaleIn(item_research, 0,null,1.0f);
        UXOptimizer.scaleIn(item_ai_ppt, 0,null,1.0f);
        UXOptimizer.scaleIn(item_simul_translate, 0,null,1.0f);
        UXOptimizer.scaleIn(item_ai_draw, 0,null,1.0f);
        UXOptimizer.scaleIn(item_ai_excel, 0,null,1.0f);
        if(needRefresh){
            if( selectFilter == AiWorkAdapter.TYPE_ALL) {
                viewModel.getVmHistory().loadAllHistory(true);
            }else if( selectFilter == AiWorkAdapter.TYPE_TRANSLATE){
