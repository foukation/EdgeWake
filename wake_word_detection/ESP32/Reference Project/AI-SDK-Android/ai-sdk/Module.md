...
dependencies {
    ...
    // SDK
    implementation(name: 'AI-SDK-release_1.1.5_', ext: 'aar')
}
```
...
val config = AIAssistConfig.Builder()
    ...
    .setDeviceNo("***")
    .setDeviceNoType("***")
    .token("***") // 接入大模型闲聊 chatbot ，需邮件申请授权，申请方式参考，Chatbot 设备端接入文档
    .setCentralConfigVersion("*") // 链路中控配置版本号，若使用此功能，请邮件申请
    .setDialogueTtsConfig(TtsConfig.DEFAULT) // 自定义 TTS 配置，也可不设置，使用默认配置
    .setVoiceGainEnabled(false) // 若使用语音助手，可选择是否开启语音增益功能，默认不开启
    .build()
...
// 6. 获取语音助手实例
val asrIntelligentDialogue = getInstance().asrIntelligentDialogueHelp() as ASRIntelligentDialogue

// 7. 获取语音助手引擎（支持语音唤醒、语音助手功能、具体使用请参考 demo）
val va = AIAssistantManager.getInstance().voiceAssistantHelp() as? VoiceAssistant
...
val config = AIAssistConfig.Builder()
    ...
    .token("token") // 接入大模型闲聊 chatbot ，需邮件申请授权，申请方式参考，Chatbot 设备端接入文档
    .setCentralConfigVersion("*") // 链路中控配置版本号，若使用此功能，请邮件申请
    .setDialogueTtsConfig(TtsConfig.DEFAULT) // 自定义 TTS 配置，也可不设置，使用默认配置
    .setVoiceGainEnabled(false) // 若使用语音助手，可选择是否开启语音增益功能，默认不开启
    .build()
...
class AIAssistExample {
    ...
    fun initializeSDK(context: Context) {
        ...
        // 获取 AI 基础功能包
        val aiFoundationKit = getInstance().aiFoundationKit() as AIFoundationKit
        // 获取 语音助手 智能对话服务（仅支持智能语音交互）
        val asrIntelligentDialogue = manager.asrIntelligentDialogueHelp() as ASRIntelligentDialogue
        // 获取语音助手引擎（支持语音唤醒、语音助手功能、具体使用请参考 demo）
        val va = manager.voiceAssistantHelp() as? VoiceAssistant
    ...
    private var asrIntelligentDialogue by lazy {
