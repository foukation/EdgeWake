...
public class SpeechRecognitionPersistentAliYunActivity extends BaseActivity implements View.OnClickListener {
    ...
    private SpeechRecognitionPersistentAliYun.ASRListener asrListener;
    ...
    private void toggleAsrRecognition() {
        ...
        if (speechRecognitionPersistent != null) {
            speechRecognitionPersistent.finish();
    ...
    private void start() {
        ...
        if (asrListener == null) {
            asrListener = new SpeechRecognitionPersistentAliYun.ASRListener() {
                @Override
                public void onConnected() {
                    // WebSocket 连接建立成功，不代表服务端已经完成识别初始化
                    long connectCostMs = SystemClock.elapsedRealtime() - connectStartMs;
                    Timber.tag(TAG).d("%s  speechRecognitionPersistent  onConnected, connectCost=%dms", this.hashCode(), connectCostMs);
                }

                @Override
                public void onAudioEnergy(float energy) {
                }

                @Override
                public void onMessageReceived(SpeechRecognitionPersistentAliYunData message) {
                    // 接收翻译结果
    ...
    private void speechRecognitionPersistent() {
        speechRecognitionPersistent.setListener(asrListener).setVad(600);
