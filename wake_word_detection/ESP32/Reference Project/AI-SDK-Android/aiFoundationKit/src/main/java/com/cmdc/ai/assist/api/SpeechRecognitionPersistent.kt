...
import com.cmdc.ai.assist.api.SpeechRecognitionPersistent.Companion.TAIL_DRAIN_MS
...
class SpeechRecognitionPersistent {
    ...
    /**
     * 开始语音识别（无业务标识）。适用于不需要区分语言模型的通用语音识别入口。
     *
     */
     ...
     * 开始语音识别（携带业务标识）。
     *
     * 作为 [startRecognitionDefault] 的重载，用于区分不同语言模型的语音识别请求。
     * @param pid 业务标识，对应 [LanguageModel] 的 pid，用于选择不同的识别接口。
     *
     */
     ...
     */
    private fun initAndConnectClient(pid: Int?) {
        /*val wsUrl =
            AssistUtils.wssParameter("wss://ivs.chinamobiledevice.com:11443/ai-admin-beta/app-ws/v1/long-asr")*/

        var url = ApiConfig.AUTOMATIC_SPEECH_RECOGNITION_PERSISTENT_API
        if (pid != null && pid == LanguageModel.CHINESE_DIALECTS_BASIC_PUNCTUATION.pid) {
            url = ApiConfig.AUTOMATIC_SPEECH_RECOGNITION_PERSISTENT_15376_API
        }

        val wsUrl =
            AssistUtils.wssParameter(url)
        ...
        try {
            ...
            Timber.tag(TAG).d("SpeechRecognitionPersistent WebSocket connect success, cost=%dms", connectCostMs)
        } catch (e: Exception) {
            val connectCostMs = SystemClock.elapsedRealtime() - connectStartMs
            Timber.tag(TAG).e(e, "SpeechRecognitionPersistent WebSocket connect failed, cost=%dms", connectCostMs)
            listener?.onError(e)
            Timber.tag(TAG).d(e)
            release()
     ...
     *    （[com.cmdc.ai.assist.aiModel.AsrClientPersistent.sendAudioDataByStream]）持续读取并上传 PCM；
