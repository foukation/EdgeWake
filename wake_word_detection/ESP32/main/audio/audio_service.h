...
/**
 * @brief Opus 编码器帧时长（毫秒）
 *
 * 不同服务对帧时长有不同要求：
 * - WebSocket/MQTT 协议服务器（小智服务）：需要使用 60ms 帧时长
 * - AI SDK 云端 ASR 服务器：需要使用 20ms 帧时长
 *   (与 Android RealTimeUploader 一致：每次编码 320 samples = 20ms @ 16kHz)
 *
 * 切换服务时需要修改此值并重新编译：
 *   - 使用小智服务：#define OPUS_FRAME_DURATION_MS 60
 *   - 使用 AI SDK：  #define OPUS_FRAME_DURATION_MS 20
 */
